...
# Ensure there are articles in the database on startup
...
if os.path.exists('news.db'):
    ...
    if count == 0:
        ...
        try:
            saved_count = update_articles()
            print(f"Fetched and saved {saved_count} initial articles.")
        except Exception as e:
    ...
    print("Database file not found. It should have been created by init_db().")
...
def index():
    ...
    # If refresh is requested, fetch new articles
    if refresh:
        update_articles()
    ...
    # Build query without source filter
    query = "SELECT * FROM articles ORDER BY published DESC LIMIT ? OFFSET ?"
    params = [per_page, offset]

    c.execute(query, params)
    articles = c.fetchall()

    # Get sources for the source links
    c.execute("SELECT DISTINCT source FROM articles ORDER BY source")
    sources = [row['source'] for row in c.fetchall()]
...
if __name__ == '__main__':
    # Fetch initial articles if the database is empty
